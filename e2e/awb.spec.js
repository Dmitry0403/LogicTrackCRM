...
test("AWB check shows short copy modal before opening terminal site", async ({ page, context, gotoWorkspace }) => {
...
test("AWB check supports empty prefix and alphanumeric main number", async ({ page, context, gotoWorkspace }) => {
  await context.grantPermissions(["clipboard-read", "clipboard-write"], { origin: "http://127.0.0.1:4173" });
  await gotoWorkspace();
  await openOrderFormForManualAwb(page, 1, {
    prefix: "",
    number: "AB12CD34",
  });
  await page.locator("#svo-terminal-shercargo").check();
  ...
  await expect.poll(async () => await page.evaluate(() => navigator.clipboard.readText())).toBe("AB12CD34");
  await expect.poll(async () => await page.evaluate(() => window.__e2eOpenedUrl)).not.toBe("");
  await expect(page.getByTestId("manual-cargo-modal")).toBeHidden();
});
